<template>
    <div class="flex w-full flex-col gap-4">
        <div
            class="rounded-sm bg-orange-200 p-4 text-center font-semibold text-orange-900 lg:hidden">
            {{ mobileWarning }}
        </div>
        ...
        <ul
            ...
            :class="{
                ...
                'mb-4': collapsible,
                'flex-wrap': collapsible,
                'justify-center xl:justify-start': collapsible,
                'xl:gap-8': collapsible,
                shrink: collapsible,
                'border-gray-100': collapsible,
                'border border-solid': collapsible,
                'xl:border-0': collapsible,
                'p-2': collapsible,
                'xl:p-0': collapsible,
            }">
            <router-link
                ...
                <li
                    @click="navigate"
                    class="cursor-pointer px-4 py-2 font-medium text-sky-800 hover:text-sky-900 hover:underline xl:-mr-2 xl:-ml-2 xl:px-2 dark:text-sky-100 dark:text-sky-200 dark:hover:text-sky-300"
                    :class="{
                        'mx-2': collapsible,
                        'bg-gray-200 dark:bg-gray-800':
                            !collapsible || isActiveLink(link),
                        'mb-2': !collapsible,
                        'text-base': $root.language === 'en',
                        'text-sm': $root.language === 'fr',
                    }">
                    <a
                        :href="href"
                        @click="navigate"
        ...
        <div
            v-if="!collapsible"
            class="mt-4 flex flex-col xl:-mr-2 xl:-ml-2 xl:px-2">
            <menu-view-featured
                ...
                <template v-slot:icon>
                    <img
                        class="mr-2 w-12 border border-solid border-gray-300 xl:w-8"
                        v-if="reportLocalization.thumbnail"
            ...
            <menu-view-featured
                ...
                <template v-slot:icon>
                    <svg
                        viewBox="0 0 20 20"
                        fill="currentColor"
                        class="database mr-2 h-12 w-12 xl:h-8 xl:w-8">
                        <path
        ...
        methods: {
            isActiveLink(link) {
                if (link.destination.name === "tableau") {
                    return (
                        link.destination.params.viewid ===
